...
string  g_sSubMenu              = "Bookmarks-OSG"; // Name of the submenu
    ...
    if (sStr == "reset") {
        ...
        if(llStringLength(sStr) > llStringLength(PLUGIN_CHAT_CMD + " save")) {
            ...
            if(llListFindList(g_lVolatile_Destinations, [sAdd]) >= 0 || llListFindList(g_lDestinations, [sAdd]) >= 0)
                ...
                // Amanda:
                llOwnerSay("Do not use auto get region"); 
                llMessageLinked(LINK_SET,NOTIFY,"0"+"Do not use auto get region",kID);
            ...
            Dialog(kID,
"Enter a hop:grid:port/region/x/y/z address for the destination below. Submit a blank field to cancel and return.", [], [], 0, iNum,"TextBoxIdSave");
...
integer validatePlace(string sStr, key kAv, integer iAuth) {
     ...
     g_tempLoc = sStr; //assign a global for use in response menu
        Dialog(kAv,
"\nEnter a name for the destination " + sStr + "
...
// Amanda:
list convertHOPurl(string sStr) {  //convert the HOP url  strings to a list
// hop://grid:port/region/x/y/z

    sStr = llStringTrim(llUnescapeURL(sStr), STRING_TRIM);
    integer len = llStringLength(sStr);

    // remove the last slash if any
    if (llGetSubString(sStr, len-1, len-1)=="/") {
        sStr = llGetSubString(sStr, 0, len-2);
    }

    list lPieces =  llParseStringKeepNulls(sStr, ["/"], []);
    string sGridPort = llList2String(lPieces, 2);
    string sRegion = llList2String(lPieces,3);


    sStr = "hop://"+sGridPort+"/"+sRegion;
    llOwnerSay("convert: "+ sStr);

    list lAddress = [sStr, llList2String(lPieces,4), llList2String(lPieces,5),llList2String(lPieces,6)];
    ...
    //take a string in region (x,y,z) format, and retrieve global coordinates.  The teleport takes place in the data server section
TeleportTo(string sStr,key kIssuer) {  

    // TODO:  Amanda parse region name
    list lRegion = convertHOPurl(sStr);

    llOwnerSay("Teleport to: " + llDumpList2String(lRegion, "~"));

    string sRegion = llList2String(lRegion,0);

    // // Extract local X, Y and Z
    g_vLocalPos.x = llList2Float(lRegion, 1);
    g_vLocalPos.y = llList2Float(lRegion, 2);
    g_vLocalPos.z = llList2Float(lRegion, 3);
    
    llOwnerSay("Teleport to: " + (string)g_vLocalPos);

    // Request the sim global coordinates
    // TODO: Amanda. RLVa @tptp:hop://grid:port/region/x/y/z:force=true does not work. And it should be nice to get it working for hypergrid
    ...
    if(g_iRLVOn == FALSE) {  //If we don't have RLV, we can just send to llMapDestination for a popup
        ...
        llRegionSayTo(kIssuer,0,"Sending "+llGetDisplayName(g_kWearer)+" to: "+sStr);
    }
            ...
            if (iMenuIndex != -1) {
                ...
                if(sMenuType == "TextBoxIdLocation") {
                    ...
                    if(sMessage != " ") {
                        ...
                        // Amanda: Do not convert to SLurl for Opensim
                        validatePlace(sMessage, kAv, iAuth);
                    }
                    else
                        UserCommand(iAuth, PLUGIN_CHAT_CMD, kAv);
