...
// -----------------------------------------
// Amanda: llMoveToTarget2 instead of directly llMoveToTarget


/**
llMoveToTarget2 replaces llMoveToTarget 
It avoids the avatar flying when walking on a prim
Inteded used is only for prims attached to avatars, like in follow scripts and collar leash
*/
llMoveToTarget2(vector target, float tau) {
    /**
    By observation:
    llMoveToTarget seems to compare the target z-component to the ground level,
    if the avatar is on an platform above the ground its z-component will be bigger than the ground level.
    This causes the avatar to trigger the flying animation.
    
    A work around for avatars walking on prims is to set the targe.z to zero.
    But this causes the avatar to only follow the target at their level, and not in the z-direction.
    It measn the avatar will remain above or beneath the target.
    */

    // A simple work around when the target is above or below the avatar
    // If the target is at the same level as the avatar then use the target.z zero. To make the avatar walk
    vector posOwn  = llGetPos();
    if (llFabs(target.z - posOwn.z) < 2) {
        target.z=0;
    }
...
// TODO:
    // Amanda: Changed to a timer to follow
    ...
    llTargetRemove(g_iTargetHandle);
    // Amanda: Release controls because we are unleashed (I think)
    llReleaseControls();
    llStopMoveToTarget();
...
YankTo(key kIn){
    llMoveToTarget2(llList2Vector(llGetObjectDetails(kIn, [OBJECT_POS]), 0), 0.5);
...
state active
{
    ...
    timer() {
        ...
        if (iIsInSimOrJustOutside && llVecDist(llGetPos(),vLeashedToPos)<(60+g_iLength)) {   //if the leasher is now in range
            ...
            if(!g_iLeasherInRange) { //and the leasher was previously not in range
                ...
                g_iTargetHandle = llTarget(g_vPos, (float)g_iLength);
                if (g_vPos != ZERO_VECTOR) llMoveToTarget2(g_vPos, 0.8);
                ApplyRestrictions();
    ...
    at_target(integer iNum, vector vTarget, vector vMe) {
        // Amanda: Release controls so the can move
        llReleaseControls();
        llStopMoveToTarget();
    ...
    not_at_target() {
        ...
        if(g_kLeashedTo) {
            ...
            if (g_vPos != ZERO_VECTOR){
                ...
                // Amanda: Request controls when Sub is not at target
                llRequestPermissions(llGetOwner(), PERMISSION_TAKE_CONTROLS);
                llMoveToTarget2(g_vPos,1.0);
            }
            else{
                // Amanda: Release controls is we stop moving
                llReleaseControls();
                llStopMoveToTarget();
                llTargetRemove(g_iTargetHandle);
            }
    ...
    run_time_permissions(integer iPerm) {
        if (iPerm & PERMISSION_TAKE_CONTROLS) {
            ...
            // Amanda:  Disable all controls when leashed and not in target
